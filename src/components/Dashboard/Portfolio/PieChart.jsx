...
class Donut extends Component {
    ...
    updatePortfolioData = (user) => {
      if (user && user.portfolio) {
        const formattedData = user.portfolio.map((item) => {
          const industry = industries.find((industry) => industry.symbol === item.stockSymbol);
          return {
            ...item,
            industryName: industry ? industry.industry : ''
          };
        });
        this.setState({ portfolioData: formattedData }, this.calculateIndustryData);
      }
    };
    ...
    calculateIndustryData = () => {
      ...
      if (portfolioData.length > 0) {
        ...
        this.setState({ industryData: industryStats }, this.calculatePieChartData);
      }
    };
  
    calculatePieChartData = () => {
  const { industryData } = this.state;
  const sortedIndustries = Object.entries(industryData).sort(([, a], [, b]) => b.totalBuyCost - a.totalBuyCost);
  const topTenIndustries = sortedIndustries.slice(0, 10);
  const otherIndustriesBuyCost = sortedIndustries.slice(10).reduce((total, [, { totalBuyCost }]) => total + totalBuyCost, 0);
  const totalBuyCostAllIndustries = sortedIndustries.reduce((total, [, { totalBuyCost }]) => total + totalBuyCost, 0);
  const topTenPercentages = topTenIndustries.map(([industry, { totalBuyCost }]) => ({
    industry,
    percentage: (totalBuyCost / totalBuyCostAllIndustries) * 100
  }));
  const labels = [...topTenPercentages.map(({ industry }) => industry), 'Others'];
  const series = [
